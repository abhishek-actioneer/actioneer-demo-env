...
const Fields = z.object({ name: z.string().trim().min(1).max(200), script: z.string().trim().min(20).max(15_000), opening: z.string().trim().min(10).max(1200), voiceId: z.uuid(), language: z.enum(["English", "Hindi", "Hinglish"]) });
