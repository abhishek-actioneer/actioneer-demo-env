...
import { BDR_TEMPLATES, getBdrTemplate, type BdrTemplateId } from "@/lib/bdr/templates";

type Fields = Pick<BdrCampaign, "name" | "script" | "opening" | "voiceId" | "language" | "templateId" | "voicemail">;
...
function fieldsOf(campaign: BdrCampaign): Fields { return { name: campaign.name, script: campaign.script, opening: campaign.opening, voiceId: campaign.voiceId, language: campaign.language, templateId: campaign.templateId, voicemail: campaign.voicemail || getBdrTemplate(campaign.templateId).voicemail }; }
...
export function BdrStudio() {
  ...
  const [templateId, setTemplateId] = useState<BdrTemplateId>("financial-services");
  ...
  async function importAudience() {
    ...
    const result = await apiFetch<{ campaign: BdrCampaign }>("/api/bdr/campaigns", { ...requestOptions, method: "POST", body: { id: importKey.current, audienceId, templateId } });
        ...
        <div className="grid items-start gap-5 lg:grid-cols-[300px_minmax(0,1fr)]">
          <aside className="space-y-5">
            <section className="rounded-lg border border-border bg-card p-5"><div className="mb-4 flex items-center justify-between"><h2 className="flex items-center gap-2 text-sm font-semibold"><Users size={16} /> Monaco segments</h2><span className="text-[11px] text-muted-foreground">{readiness.monaco ? "Connected" : "Setup needed"}</span></div>
              ...
              <label className="mt-3 block text-xs font-medium">Script template<select className={`${control} mt-2`} value={templateId} disabled={!!busy} onChange={(e) => { setTemplateId(e.target.value as BdrTemplateId); importKey.current = ""; }}>{BDR_TEMPLATES.map((template) => <option key={template.id} value={template.id}>{template.name}</option>)}</select></label>
          ...
          {!selected || !fields ? <section className="flex min-h-[540px] flex-col items-center justify-center rounded-lg border border-dashed border-border px-8 text-center"><Workflow className="mb-5 text-muted-foreground" size={38} /><h2 className="text-xl font-medium">Build your first calling campaign</h2><p className="mt-3 max-w-sm text-sm leading-6 text-muted-foreground">Connect Monaco and import a segment to get started. Your script, voice, contacts, and call progress will live here.</p><p className="mt-6 text-xs text-muted-foreground">1. Import segment &nbsp; → &nbsp; 2. Save script &nbsp; → &nbsp; 3. Launch</p></section> : <section className="min-w-0 overflow-hidden rounded-lg border border-border bg-card">
            ...
            {tab === "script" ? <div className="space-y-5 p-5 md:p-6">
              <div className="rounded-md border border-border p-4"><p className="text-sm font-medium">Start with a focused conversation</p><p className="mt-1 text-xs leading-5 text-muted-foreground">Applying a template replaces the opening, script, and voicemail in this editor. Review and save before calling.</p><div className="mt-3 grid gap-3 sm:grid-cols-2">{BDR_TEMPLATES.map((template) => <button key={template.id} disabled={!!locked || !!busy} className={`${button} flex-col items-start text-left ${fields.templateId === template.id ? "bg-muted" : ""}`} onClick={() => { setFields({ ...fields, templateId: template.id, script: template.script, opening: template.opening, voicemail: template.voicemail }); setMessage(`${template.name} applied. Review and save your changes.`); }}><span>{template.name}</span><span className="text-xs font-normal leading-5 text-muted-foreground">{template.description}</span><span className="text-[11px] underline">Apply template</span></button>)}</div></div>
              ...
              <label className="block text-xs font-medium">Voicemail message<textarea className={`${control} mt-2 min-h-28 resize-y leading-6`} maxLength={1600} disabled={!!locked} value={fields.voicemail || ""} onChange={(e) => setFields({ ...fields, voicemail: e.target.value })} /><span className="mt-2 block text-[11px] font-normal leading-5 text-muted-foreground">A concise message for voicemail. Call screening gets “This is Daniel from Actioneer,” then Daniel waits for the person.</span></label>
...
function CallDetails({ recipient, onClose }: { recipient: BdrRecipient; onClose: () => void }) {
  return <div className="mt-6 rounded-md border border-border p-4">
    ...
    <div className="mb-4"><CallSentiment recipient={recipient} />{recipient.callOutcome && <p className="mt-2 text-xs capitalize text-muted-foreground">{recipient.callOutcome === "screening" ? "Call screening — no conversation" : recipient.callOutcome}</p>}{recipient.followUpRequested && <p className="mt-2 text-xs">Team follow-up requested. Arrange it with your team; no message or appointment has been scheduled automatically.</p>}</div>
    {recipient.transcript?.map((turn, i) => <div key={i} className={`mb-3 rounded-md p-3 text-sm ${turn.role === "assistant" ? "bg-muted" : "border border-border"}`}>
      <p className="mb-1 text-[11px] font-medium text-muted-foreground">{turn.role === "assistant" ? "Daniel" : turn.source === "screening" ? "Call screening" : turn.source === "voicemail" ? "Voicemail greeting" : "Prospect"}{turn.delivery === "interrupted" ? " · Interrupted; playback may be incomplete" : ""}</p>
      {turn.text}
    </div>)}
...
function CallSentiment({ recipient }: { recipient: BdrRecipient }) {
  if (recipient.sentiment) return <div className="mt-2"><span className="rounded border border-border px-2 py-1 text-[11px] capitalize">{recipient.sentiment.label}</span><p className="mt-2 max-w-64 text-[11px] text-muted-foreground">{recipient.sentiment.reason}</p></div>;
  if (!recipient.callId || !["completed", "failed", "no_answer"].includes(recipient.status)) return null;
  const label = recipient.sentimentState === "not_applicable" ? "No sentiment — insufficient conversation" : recipient.sentimentState === "failed" ? "Sentiment unavailable" : "Sentiment pending";
